...
export async function createAction(body: Record<string, unknown>, reply: FastifyReply) {
  try {
    const { userId, companyId, action, entityType, entityId, details, ipAddress, userAgent } = body
    if (!userId || !action || !entityType) {
      reply.code(400)
      return { success: false as const, error: 'userId, action y entityType son requeridos' }
    }
    ...
    reply.code(500)
    return { success: false as const, error: 'Error al registrar acción', message: error instanceof Error ? error.message : 'Error desconocido' }
  }
}

export async function listActions(query: ActionHistoryQuery, reply: FastifyReply) {
  try {
    ...
    return {
      ...
      data: { actions, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } },
    }
  } catch (error) {
    reply.code(500)
    return { success: false as const, error: 'Error al obtener historial de acciones', message: error instanceof Error ? error.message : 'Error desconocido' }
  }
}
