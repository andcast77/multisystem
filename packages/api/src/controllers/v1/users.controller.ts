...
export async function registerRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/v1/users', { preHandler: usersGuard }, (request, reply) => list(request, reply))
  fastify.get<{ Params: { id: string } }>('/v1/users/:id', { preHandler: usersGuard }, (request, reply) => getById(request, reply))
  fastify.post('/v1/users', { preHandler: usersGuard }, (request, reply) => create(request, reply))
  fastify.put<{ Params: { id: string }; Body: unknown }>('/v1/users/:id', { preHandler: usersGuard }, (request, reply) => update(request, reply))
  fastify.delete<{ Params: { id: string } }>('/v1/users/:id', { preHandler: usersGuard }, (request, reply) => remove(request, reply))
}
