...
export function registerRoutes(fastify: FastifyInstance) {
  fastify.get('/v1/shopflow/categories', { preHandler: pre }, handle(listCategories))
  fastify.get<{ Params: { id: string } }>('/v1/shopflow/categories/:id', { preHandler: pre }, handle(getCategoryById))
  fastify.post('/v1/shopflow/categories', { preHandler: pre }, handle(createCategory))
  fastify.put<{ Params: { id: string } }>('/v1/shopflow/categories/:id', { preHandler: pre }, handle(updateCategory))
  fastify.delete<{ Params: { id: string } }>('/v1/shopflow/categories/:id', { preHandler: pre }, handle(deleteCategory))
}
