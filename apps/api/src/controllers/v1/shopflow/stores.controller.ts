...
export function registerRoutes(fastify: FastifyInstance) {
  fastify.get('/v1/shopflow/stores', { preHandler: pre }, handle(listStores))
  fastify.get<{ Params: { code: string } }>('/v1/shopflow/stores/by-code/:code', { preHandler: pre }, handle(getStoreByCode))
  fastify.get<{ Params: { id: string } }>('/v1/shopflow/stores/:id', { preHandler: pre }, handle(getStoreById))
  fastify.post('/v1/shopflow/stores', { preHandler: pre }, handle(createStore))
  fastify.put<{ Params: { id: string } }>('/v1/shopflow/stores/:id', { preHandler: pre }, handle(updateStore))
  fastify.delete<{ Params: { id: string } }>('/v1/shopflow/stores/:id', { preHandler: pre }, handle(deleteStore))
}
