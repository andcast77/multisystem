...
export function registerRoutes(fastify: FastifyInstance) {
  fastify.get<{ Params: { userId: string } }>('/v1/shopflow/user-preferences/:userId', { preHandler: pre }, handle(getUserPreferences))
  fastify.put<{ Params: { userId: string } }>('/v1/shopflow/user-preferences/:userId', { preHandler: pre }, handle(updateUserPreferences))
}
