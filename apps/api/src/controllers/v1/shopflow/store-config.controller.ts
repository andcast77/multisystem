...
export function registerRoutes(fastify: FastifyInstance) {
  fastify.get('/v1/shopflow/store-config', { preHandler: pre }, handle(getStoreConfig))
  fastify.put('/v1/shopflow/store-config', { preHandler: pre }, handle(updateStoreConfig))
  fastify.post('/v1/shopflow/store-config/next-invoice-number', { preHandler: pre }, handle(nextInvoiceNumber))
  fastify.get('/v1/shopflow/ticket-config', { preHandler: pre }, handle(getTicketConfig))
  fastify.put('/v1/shopflow/ticket-config', { preHandler: pre }, handle(updateTicketConfig))
}
