/**
 * Vercel API handler at repo root. Forwards all requests to the Fastify app.
 * Rewrite "/(.*)" -> "/api/$1" in vercel.json so this receives every request.
 */
...
function headersFromFastify(headers: Record<string, string | string[] | undefined>): Headers {
  ...
  for (const [key, value] of Object.entries(headers)) {
    ...
    if (Array.isArray(value)) {
      value.forEach((v: string) => out.append(key, v))
    } else {
      out.set(key, value)
    }
  }
